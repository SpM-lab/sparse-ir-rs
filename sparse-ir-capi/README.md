...
# (requires cbindgen: install with `cargo install cbindgen`)
cargo build --release
...
The header file `include/sparseir/sparseir.h` is automatically generated by `cbindgen` during `cargo build` with C++ compatibility enabled (`--cpp-compat` flag). Note that `cbindgen` must be installed (`cargo install cbindgen`); if it is not found, the build will skip header generation with a warning. The header is used directly without post-processing.
