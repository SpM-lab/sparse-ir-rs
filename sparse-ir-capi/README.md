...
The `Cargo.toml` is configured with `generation = false` so that `cargo cinstall` uses the pre-processed header from `assets/sparse_ir_capi.h`. This file is a copy of `include/sparseir/sparseir.h` and is maintained in the repository (not generated by `build.rs` to avoid `cargo publish` issues).
