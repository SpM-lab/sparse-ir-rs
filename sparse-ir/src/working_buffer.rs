...
/// Working context for in-place sampling operations
///
/// This structure holds reusable buffers for evaluate/fit operations,
/// avoiding repeated memory allocations when performing multiple operations.
///
/// # Example
/// ```ignore
/// use sparse_ir::working_buffer::SamplingContext;
/// use sparse_ir::TauSampling;
///
/// let sampling = TauSampling::new(...);
/// let mut ctx = SamplingContext::new();
///
/// // Multiple operations reuse the same buffers
/// sampling.evaluate_nd_with_context(&mut ctx, &coeffs, dim, &mut output);
/// sampling.fit_nd_with_context(&mut ctx, &values, dim, &mut coeffs);
/// ```
pub struct SamplingContext {
    /// Buffer for movedim input (before GEMM)
    input_buffer: WorkingBuffer,
    /// Buffer for movedim output (after GEMM)
    output_buffer: WorkingBuffer,
}

impl SamplingContext {
    /// Create a new sampling context with empty buffers
    pub fn new() -> Self {
        Self {
            input_buffer: WorkingBuffer::new(),
            output_buffer: WorkingBuffer::new(),
        }
    }

    /// Create a new sampling context with pre-allocated buffers
    ///
    /// # Arguments
    /// * `input_capacity` - Number of elements for input buffer
    /// * `output_capacity` - Number of elements for output buffer
    pub fn with_capacity<T>(input_capacity: usize, output_capacity: usize) -> Self {
        let mut input_buffer = WorkingBuffer::new();
        let mut output_buffer = WorkingBuffer::new();
        input_buffer.ensure_capacity::<T>(input_capacity);
        output_buffer.ensure_capacity::<T>(output_capacity);
        Self {
            input_buffer,
            output_buffer,
        }
    ...
    pub fn input_buffer_mut(&mut self) -> &mut WorkingBuffer {
        &mut self.input_buffer
    }

    /// Get mutable access to the output buffer
    pub fn output_buffer_mut(&mut self) -> &mut WorkingBuffer {
        &mut self.output_buffer
    }

    /// Ensure both buffers have sufficient capacity for the given element counts
    pub fn ensure_capacity<T>(&mut self, input_count: usize, output_count: usize) {
        self.input_buffer.ensure_capacity::<T>(input_count);
        self.output_buffer.ensure_capacity::<T>(output_count);
    }

    /// Get input buffer as f64 slice
    ///
    /// # Safety
    /// Caller must ensure the buffer has sufficient capacity
    pub unsafe fn input_as_f64_mut(&mut self, count: usize) -> &mut [f64] {
        self.input_buffer.as_f64_slice_mut(count)
    }

    /// Get output buffer as f64 slice
    ///
    /// # Safety
    /// Caller must ensure the buffer has sufficient capacity
    pub unsafe fn output_as_f64_mut(&mut self, count: usize) -> &mut [f64] {
        self.output_buffer.as_f64_slice_mut(count)
    }

    /// Get input buffer as Complex<f64> slice
    ///
    /// # Safety
    /// Caller must ensure the buffer has sufficient capacity
    pub unsafe fn input_as_complex_mut(
        &mut self,
        count: usize,
    ) -> &mut [num_complex::Complex<f64>] {
        self.input_buffer.as_complex_slice_mut(count)
    }

    /// Get output buffer as Complex<f64> slice
    ///
    /// # Safety
    /// Caller must ensure the buffer has sufficient capacity
    pub unsafe fn output_as_complex_mut(
        &mut self,
        count: usize,
    ) -> &mut [num_complex::Complex<f64>] {
        self.output_buffer.as_complex_slice_mut(count)
    }
}
...
mod tests {
    ...
    fn test_sampling_context_new() {
        ...
        assert_eq!(ctx.input_buffer.capacity_bytes(), 0);
        assert_eq!(ctx.output_buffer.capacity_bytes(), 0);
    }

    #[test]
    fn test_sampling_context_ensure_capacity() {
        let mut ctx = SamplingContext::new();
        ctx.ensure_capacity::<f64>(100, 200);

        assert!(ctx.input_buffer.capacity_bytes() >= 100 * std::mem::size_of::<f64>());
        assert!(ctx.output_buffer.capacity_bytes() >= 200 * std::mem::size_of::<f64>());
    }

    #[test]
    fn test_sampling_context_buffers() {
        let mut ctx = SamplingContext::new();
        ctx.ensure_capacity::<f64>(10, 20);

        unsafe {
            // Write to input buffer
            let input = ctx.input_as_f64_mut(10);
            for i in 0..10 {
                input[i] = i as f64;
            }

            // Write to output buffer
            let output = ctx.output_as_f64_mut(20);
            for i in 0..20 {
                output[i] = (i * 2) as f64;
            }
    ...
    #[test]
    fn test_sampling_context_complex_buffers() {
        let mut ctx = SamplingContext::new();
        ctx.ensure_capacity::<Complex<f64>>(5, 10);

        unsafe {
            let input = ctx.input_as_complex_mut(5);
            for i in 0..5 {
                input[i] = Complex::new(i as f64, (i + 1) as f64);
            }

            let output = ctx.output_as_complex_mut(10);
            for i in 0..10 {
                output[i] = Complex::new((i * 2) as f64, (i * 3) as f64);
            }

            let input = ctx.input_as_complex_mut(5);
            assert_eq!(input[2], Complex::new(2.0, 3.0));

            let output = ctx.output_as_complex_mut(10);
            assert_eq!(output[5], Complex::new(10.0, 15.0));
        }
    }
}
