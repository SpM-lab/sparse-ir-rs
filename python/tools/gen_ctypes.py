...
def main():
    ...
    # Try to find header in Rust location (auto-generated by cbindgen)
    # Then fall back to copied location in pylibsparseir (after build)
    header_candidates = [
        project_root / 'sparse-ir-capi' / 'include' / 'sparseir' / 'sparseir.h',  # Rust source location (auto-generated)
        python_dir / 'pylibsparseir' / 'sparseir.h',  # Copied location (after build)
        python_dir / 'include' / 'sparseir' / 'sparseir.h',  # Legacy copied location
    ]
    ...
    if not header_path:
        ...
        for candidate in header_candidates:
            print(f"  {candidate}")
        print("\nNote: The header is auto-generated by cbindgen during Rust build.")
        print("Make sure to build sparse-ir-capi first: cd ../sparse-ir-capi && cargo build")
        sys.exit(1)

    output_path = python_dir / 'pylibsparseir' / 'ctypes_autogen.py'

    # Include directories - prioritize Rust location
    include_dirs = [
        str(project_root / 'sparse-ir-capi' / 'include'),  # Rust location
        str(project_root / 'sparse-ir-capi' / 'include' / 'sparseir'),
        str(python_dir / 'include'),  # Legacy copied location
