...
use mdarray::{DTensor, DSlice, Layout};
...
where
    T: num_complex::ComplexFloat + faer_traits::ComplexField + num_traits::One + num_traits::Zero + Copy + 'static,
{
    let (_m, k) = *a.shape();
    let (k2, _n) = *b.shape();
    ...
    // Create result tensor - using from_elem with T::zero() for safe memory management
    // Note: matmul_par_overwrite will overwrite all elements, so zero initialization
    // is just for safety and will be immediately overwritten
    let mut result = DTensor::<T, 2>::from_elem([_m, _n], T::zero());
