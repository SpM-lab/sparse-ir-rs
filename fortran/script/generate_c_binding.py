...
def sanitize_fortran_name(name: str) -> str:
    """Ensure the identifier is valid in Fortran (must start with a letter)."""
    if not name:
        return "arg"
...
def generate_fortran_interface(cursor, types):
    ...
    for arg in cursor.get_arguments():
        raw_name = arg.spelling or 'arg'
        name = sanitize_fortran_name(raw_name)
    ...
    # Track which iso_c_binding symbols are needed
    import_types = set()
    ...
    def add_imports_from_ftype(ftype: str):
        ...
        if 'c_double' in ftype:
            import_types.add('c_double')
        if 'c_int64_t' in ftype:
            import_types.add('c_int64_t')
        if 'c_int32_t' in ftype:
            import_types.add('c_int32_t')
        if 'c_int' in ftype:
            import_types.add('c_int')

    for decl in decls:
        add_imports_from_ftype(decl)

    # Include return type (for functions) in import detection
    if result_type != 'subroutine':
        add_imports_from_ftype(result_type)

    import_stmt = ""
    if import_types:
        import_stmt = f"  import :: {', '.join(sorted(import_types))}"

    def format_declaration(kind_keyword: str):
        max_line_length = 90
        if not args:
            return f"{kind_keyword} {fortran_func_name}() &"
        lines = [f"{kind_keyword} {fortran_func_name}( &"]
        current_args = []
        for idx, arg in enumerate(args):
            test_args = current_args + [arg]
            line_candidate = "    & " + ", ".join(test_args)
            if current_args and len(line_candidate) > max_line_length:
                lines.append("    & " + ", ".join(current_args) + ", &")
                current_args = [arg]
            else:
                current_args = test_args
        lines.append("    & " + ", ".join(current_args) + ") &")
        return "\n".join(lines)

    spec_sections = []
    if import_stmt:
        spec_sections.append(import_stmt)
    if decl_lines:
        spec_sections.append(decl_lines)
    ...
    if result_type == 'subroutine':
        decl_header = format_declaration("subroutine")
        c_binding = f"""
{decl_header}
    bind(c, name="{func_name}")
{specs_text}end subroutine
""".strip()
    else:
        result_decl = f"  {result_type.split(',')[0]} :: {fortran_func_name}"
        # Ensure result declaration is included in specs
        result_specs = specs_text + result_decl + "\n"
        decl_header = format_declaration("function")
        c_binding = f"""
{decl_header}
    bind(c, name="{func_name}")
{result_specs}
...
def main():
    ...
    with open(os.path.join(output_dir, "_cbinding.inc"), 'w') as f:
        f.write("! Autogenerated Fortran interfaces for " + header_path + "\n")
        for code in c_interfaces:
            f.write(code + "\n\n")

    # Write C binding public declarations
    with open(os.path.join(output_dir, "_cbinding_public.inc"), 'w') as f:
