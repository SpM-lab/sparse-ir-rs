...
def generate_evaluate_matsubara_function(ndim, input_type, output_type):
  ...
  SELECT CASE (statistics)
  CASE (SPIR_STATISTICS_FERMIONIC)
    status_c = {c_function}(obj%matsu_f_smpl_ptr, obj%backend_ptr, SPIR_ORDER_COLUMN_MAJOR, &
      ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr), c_loc(res))
  CASE (SPIR_STATISTICS_BOSONIC)
    status_c = {c_function}(obj%matsu_b_smpl_ptr, obj%backend_ptr, SPIR_ORDER_COLUMN_MAJOR, &
      ndim_c, c_loc(input_dims_c), target_dim_c, c_loc(arr), c_loc(res))
  CASE DEFAULT
...
def main():
    ...
    output_file = os.path.join(output_dir, "evaluate_matsubara_impl.inc")
